...
public:
    ...
    StateStackCompartment(const StateStackCompartment& other)
    {
        state = other.state;
        stateArgs = other.stateArgs;
        stateVars = other.stateVars;
        enterArgs = other.enterArgs;
        exitArgs = other.exitArgs;
        _forwardEvent = other._forwardEvent;
    }
};
...
public:
    StateStack()
    {
        ...
        this->_stateStack_ = new std::stack<StateStackCompartment>();
...
public:
    void to_a()
    {
        FrameEvent e("to_a", std::unordered_map<std::string, std::any>());
        this->_mux_(&e);
    }

    void to_b()
    {
        FrameEvent e("to_b", std::unordered_map<std::string, std::any>());
        this->_mux_(&e);
    }

    void to_c()
    {
        FrameEvent e("to_c", std::unordered_map<std::string, std::any>());
        this->_mux_(&e);
    }

    void push()
    {
        FrameEvent e("push", std::unordered_map<std::string, std::any>());
        this->_mux_(&e);
    }

    void pop()
    {
        FrameEvent e("pop", std::unordered_map<std::string, std::any>());
        this->_mux_(&e);
    }

    void pop_change()
    {
        FrameEvent e("pop_change", std::unordered_map<std::string, std::any>());
        this->_mux_(&e);
    }
...
private:
    void _sA_(FrameEvent *e)
    {
        ...
        else if (e->_message == "pop")
        {
            ...
            std::cout << "POP State: " << compartment->state << std::endl;
            this->_transition_(compartment);
            return;
        }
        ...
        {
            StateStackCompartment *compartment = this->_stateStack_pop_();
            std::cout << "POP CHANGE State: " << compartment->state << std::endl;
...
private:
    std::stack<StateStackCompartment> *_stateStack_ = nullptr;

public:
    void _stateStack_push_(StateStackCompartment *compartment)
    {
        _stateStack_->push(*compartment);
        StateStackCompartment* copyPtr = new StateStackCompartment(*compartment);
    ...
    StateStackCompartment *_stateStack_pop_()
    {
        ...
        _stateStack_->pop();
        StateStackCompartment* copyPtr = new StateStackCompartment(copyCompartment);
...
private:
    void _changeState_(StateStackCompartment *compartment)
    {
