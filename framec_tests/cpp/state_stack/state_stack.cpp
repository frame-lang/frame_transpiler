...
public:
    ...
    StateStackCompartment(int state) : state(state) {}

    // Copy constructor for deep copy
    StateStackCompartment(const StateStackCompartment &other)
        : state(other.state), stateArgs(other.stateArgs), stateVars(other.stateVars),
          enterArgs(other.enterArgs), exitArgs(other.exitArgs), _forwardEvent(nullptr)
    {
    }

    // Assignment operator for deep copy
    StateStackCompartment &operator=(const StateStackCompartment &other)
    {
        if (this != &other)
        {
            state = other.state;
...
private:
    ...
    stack<StateStackCompartment> *_stateStack_; 
...
private:
    ...
    {
        _nextCompartment_ =  new StateStackCompartment(*compartment);
    }
    ...
    public:
    // Copy constructor for deep copy
    StateStack(const StateStack &other)
        : _compartment_(nullptr), _nextCompartment_(nullptr)
    {
        *this = other;
    }

    // Assignment operator for deep copy
    StateStack &operator=(const StateStack &other)
    {
        if (this != &other)
        {
            _state_ = other._state_;
            delete _compartment_;
            _compartment_ = new StateStackCompartment(*other._compartment_);
            delete _nextCompartment_;
            if (other._nextCompartment_ != nullptr)
                _nextCompartment_ = new StateStackCompartment(*other._nextCompartment_);
            else
                _nextCompartment_ = nullptr;
            _stateStack_ = other._stateStack_; 
        }
        return *this;
    }
