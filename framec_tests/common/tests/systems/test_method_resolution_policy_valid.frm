...
fn main() {
    var sys = MethodResolutionSystem()
    ...
    machine:
        $Ready {
            test_all_valid_calls() {
                ...
                var result_a = self.operation_method_a()
                print("Got: " + result_a)
                var result_b = self.operation_method_b(10, 20)
                print("Got: " + str(result_b))
                var result_c = self.operation_only_method()
                print("Got: " + str(result_c))
                
                # 3. Valid system.interface calls
                print("3. Interface calls:")
                var interface_result_a = system.interface_method_a()
                print("Interface result: " + interface_result_a)
                var interface_result_b = system.interface_method_b(100)
                print("Interface result: " + str(interface_result_b))
                
                # 4. Valid static operation calls
                print("4. Static operation calls:")
                var static_result = MethodResolutionSystem.static_operation(5, 6)
            ...
            interface_method_a(): string {
                ...
                self.action_method_a()
                var op_result = self.operation_method_a()
            ...
            interface_method_b(x: int): int {
                ...
                var calc_result = self.operation_method_b(x, x)
