...
# Expected: User functions can coexist with imports without special library helpers

# Test 1: User-defined function with same name as builtins
def str(x):
...
def test_user_defined_shadows():
    print("=== Testing user-defined shadows (no special libs) ===")
    num = 42
...
def test_multiple_import_styles():
