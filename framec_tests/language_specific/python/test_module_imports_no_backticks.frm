...
fn test_module_functions() {
    ...
    sqrt_val = math.sqrt(25)           # Was: math.sqrt(25)
    abs_val = math.fabs(-10.5)         # Was: math.fabs(-10.5)
...
fn test_nested_module_calls() {
    ...
    now = dt.datetime.now()            # Was: dt.datetime.now()
    exists = path.exists("/tmp")       # Was: path.exists('/tmp')
...
fn test_complex_expressions() {
    ...
    ceil_val = math.ceil(4.3)          # Was: math.ceil(4.3)
    floor_val = math.floor(4.7)        # Was: math.floor(4.7)
