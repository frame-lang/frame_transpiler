...
system FileIOTest {
    actions:
        testFileOperations() {
            ...
            var f = open("test_input.txt", "w")
            f.write("Frame File I/O Test Data")
            f.close()
            
            # Test file existence using Python os.path
            var exists = os.path.exists("test_input.txt")
            print(f"File exists: {exists}")
            
            # Read file content using Python idioms
            if exists {
                var f_read = open("test_input.txt", "r")
                var content = f_read.read().strip()
                f_read.close()
            ...
            var f_write = open("test_output.txt", "w")
            f_write.write("Hello from Frame Python!")
            f_write.close()
            
            # Verify write succeeded
            var write_exists = os.path.exists("test_output.txt")
