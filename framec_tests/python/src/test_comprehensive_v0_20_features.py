...
class AdvancedProcessor:
    ...
    def __advancedprocessor_state_Idle(self, __e, compartment):
        if __e._message == "$>":# DEBUG_EXPR_TYPE: Discriminant(4)
            ...
            if __e._parameters["mode"] == "debug":# DEBUG_EXPR_TYPE: Discriminant(4)
                
                print("Enabling debug mode")# DEBUG: TransitionStmt
                
                next_compartment = FrameCompartment('__advancedprocessor_state_Debug', None, None, None, None)
                self.__transition(next_compartment)
            elif __e._parameters["mode"] == "fast":# DEBUG_EXPR_TYPE: Discriminant(4)
                
                print("Enabling fast mode")# DEBUG: TransitionStmt
    ...
    def __advancedprocessor_state_Processing(self, __e, compartment):
        if __e._message == "$>":# DEBUG_EXPR_TYPE: Discriminant(4)
            ...
            if result == "error":
                self.return_stack[-1] = "processing failed"# DEBUG: TransitionStmt
                
                next_compartment = FrameCompartment('__advancedprocessor_state_Idle', None, None, None, None)
                self.__transition(next_compartment)
            elif result == "warning":
                self.return_stack[-1] = "processed with warnings"# DEBUG: TransitionStmt
                
                next_compartment = FrameCompartment('__advancedprocessor_state_Idle', None, None, None, None)
                self.__transition(next_compartment)
            else:
                self.return_stack[-1] = "success: " + result# DEBUG: TransitionStmt
            ...
            print("Resetting from processing state")# DEBUG: TransitionStmt
    ...
    def __advancedprocessor_state_Debug(self, __e, compartment):
        if __e._message == "$>":# DEBUG_EXPR_TYPE: Discriminant(4)
            ...
            return
        elif __e._message == "processData":# DEBUG_EXPR_TYPE: Discriminant(4)
            ...
            print("Exiting debug mode")# DEBUG: TransitionStmt
    ...
    def __advancedprocessor_state_FastProcessing(self, __e, compartment):
        if __e._message == "processData":
            self.return_stack[-1] = "fast: " + __e._parameters["input"]# DEBUG: TransitionStmt
            
            next_compartment = FrameCompartment('__advancedprocessor_state_Idle', None, None, None, None)
            self.__transition(next_compartment)
            return
        elif __e._message == "reset":# DEBUG: TransitionStmt
