...
func TestNestedIf(t *testing.T) {
	...
	sm.D()
	sm.OnInt(50)
	assert.Equal(t, x._compartment_.State, BranchState_F1)
	assert.Equal(t, []string{"> 0", "< 100"}, x.tape)
	sm = NewBranch()
	x = sm.(*branchStruct)
	sm.D()
	sm.OnInt(200)
	assert.Equal(t, x._compartment_.State, BranchState_NestedIf)
	assert.Equal(t, []string{"> 0", ">= 100"}, x.tape)
	sm = NewBranch()
	x = sm.(*branchStruct)
	sm.D()
	...
	sm.D()
	sm.OnInt(-10)
	assert.Equal(t, x._compartment_.State, BranchState_F2)
	assert.Equal(t, []string{"<= 0", "<= -10"}, x.tape)
}
...
func TestGuardedTransition(t *testing.T) {
	...
	sm.E()
	sm.OnInt(5)
	assert.Equal(t, x._compartment_.State, BranchState_F3)
