...
impl PythonVisitor {
    ...
    pub fn new(   arcanium:Arcanum
                  ...
                  , comments:Vec<Token>) -> PythonVisitor {
    ...
    fn format_variable_expr(&mut self, variable_node:&VariableNode) -> String {
        ...
        match variable_node.scope {
            ...
            IdentifierDeclScope::StateParam => {
                ...
                code.push_str(&format!("self._stateContext_.getStateArg(\"{}\")"
            ...
            IdentifierDeclScope::StateVar => {
                ...
                code.push_str(&format!("self._stateContext_.getStateVar(\"{}\")"
                                       ,variable_node.id_node.name.lexeme));
                if self.visiting_call_chain_literal_variable {
                    code.push_str(")");
                }
 ...
 //               self.newline();
                ...
                if self.generate_state_context {
                    if self.generate_exit_args {
                        self.add_code(&format!("def _transition_(self, newState, exitArgs, stateContext):"));
                    } else {
                        self.add_code(&format!("def _transition_(self, newState, stateContext):"));
                    }
                } else {
                    if self.generate_exit_args {
                        self.add_code(&format!("def _transition_(self, newState, exitArgs):"));
                    } else {
                        self.add_code(&format!("def _transition_(self, newState):"));
                    }
                }
                self.indent();
                self.newline();
                if self.generate_exit_args {
                    self.add_code(&format!("exitEvent = FrameEvent(\"<\",exitArgs)"));
                } else {
                    self.add_code(&format!("exitEvent = FrameEvent(\"<\",None)"));
                }
                self.newline();
                self.add_code(&format!("self._state_(exitEvent)"));
                self.newline();
                ...
                if self.generate_state_context {
                    ...
                    self.newline();
                    self.add_code(&format!("enterEvent = FrameEvent(\">\",self._stateContext_.getEnterArgs())"));
                    self.newline();
                } else {
                    self.add_code(&format!("enterEvent = FrameEvent(\">\",None)"));
                    self.newline();
                }
                self.add_code(&format!("self._state_(enterEvent)"));
                self.outdent();
                self.newline();
            ...
            if self.generate_state_stack {
                ...
                if self.generate_state_context {
                    self.newline();
                    self.newline();
                    self.add_code(&format!("def _stateStack_push_(self, stateContext):"));
                    self.indent();
                    self.newline();
         ...
         //           self.add_code(&format!("}}"));
                    self.newline();
                    self.newline();
                    self.add_code(&format!("def _stateStack_pop_(self):"));
                    self.indent();
                    self.newline();
                    ...
                    self.newline();
                    self.newline();
                    self.add_code(&format!("def _stateStack_push_(self, state):"));
                    self.indent();
                    self.newline();
...
//                self.add_code(&format!("}}"));
            ...
            if self.generate_change_state {
                self.newline();
                self.newline();
                self.add_code(&format!("def _changeState_(newState):"));
                self.indent();
                self.newline();
                self.add_code(&format!("self._state_ = newState"));
                self.outdent();
...
//                self.add_code(&format!("}}"));
    ...
    fn generate_state_ref_change_state(&mut self, change_state_stmt_node: &ChangeStateStatementNode) {
        ...
        self.newline();
        self.add_code(&format!("self._changeState_({});", self.format_target_state_name(target_state_name)));
    ...
    fn generate_state_ref_transition(&mut self, transition_statement: &TransitionStatementNode) {
        ...
        if self.generate_state_context {
            self.add_code(&format!("stateContext = StateContext(self.{})", self.generate_state_ref_code(&target_state_name)));
            self.newline();
        }
...
//                code = target_state_vars.clone();
        ...
        if self.generate_state_context {
            if self.generate_exit_args {
                self.add_code(&format!("self._transition_(self.{},{},stateContext)",self.format_target_state_name(target_state_name),exit_args));
            } else {
                self.add_code(&format!("self._transition_(self.{},stateContext)",self.format_target_state_name(target_state_name)));
            }
        } else {
            if self.generate_exit_args {
                self.add_code(&format!("self._transition_(self.{},{})",self.format_target_state_name(target_state_name),exit_args));
            } else {
                self.add_code(&format!("self._transition_(self.{})",self.format_target_state_name(target_state_name)));
            }
        }
    }
    ...
    fn generate_state_stack_pop_transition(&mut self, transition_statement: &TransitionStatementNode) {
        ...
        if self.generate_state_context {
            self.add_code(&format!("stateContext = self._stateStack_pop_()"));
        } else {
            self.add_code(&format!("state = self._stateStack_pop_()"));
        }
        self.newline();
        if self.generate_exit_args {
            if self.generate_state_context {
                self.add_code(&format!("self._transition_(stateContext.state,exitArgs,stateContext)"));
            } else {
                self.add_code(&format!("self._transition_(state,exitArgs)"));
            }
        } else {
            if self.generate_state_context {
                self.add_code(&format!("self._transition_(stateContext.state,stateContext)"));
            } else {
...
impl AstVisitor for PythonVisitor {
    ...
    fn visit_system_node(&mut self, system_node: &SystemNode) -> AstVisitorReturnType {
        ...
        if self.has_states {
            self.newline();
            self.add_code(&format!("def __init__(self):"));
            self.indent();
            self.newline();
            self.add_code(&format!("self._state_ = self._s{}_", self.first_state_name));
            if self.generate_state_context {
                self.newline();
                self.add_code(&format!("self._stateContext_ = StateContext(self._s{}_)", self.first_state_name));
                if let Some(state_symbol_rcref) = self.arcanium.get_state(&self.first_state_name) {
                    ...
                    if state_node.vars.is_some() {
                        for var_rcref in state_node.vars.as_ref().unwrap() {
                            ...
                            expr_t.accept_to_string(self, &mut expr_code);
                            self.newline();
                            self.add_code(&format!("self._stateContext_.addStateVar(\"{}\",{})", var.name, expr_code));
                        }
                    }
        ...
        self.subclass_code.push(format!("#class {}Controller({}):",system_node.name,system_node.name));
    ...
    fn visit_interface_method_node(&mut self, interface_method_node: &InterfaceMethodNode) -> AstVisitorReturnType {
        ...
        self.newline();
        self.add_code(&format!("e = FrameEvent(\"{}\",{})", method_name_or_alias,params_param_code));
        self.newline();
    ...
    fn visit_machine_block_node(&mut self, machine_block_node: &MachineBlockNode) -> AstVisitorReturnType {
        ...
        self.deserialize.push("".to_string());
        self.deserialize.push("\tbag = JSON.parse(data)".to_string());
        self.deserialize.push("".to_string());
    ...
    fn visit_state_node(&mut self, state_node: &StateNode) -> AstVisitorReturnType {
        ...
        self.serialize.push(format!("\tif (self._state_ == _s{}_) stateName = \"{}\"",state_node.name,state_node.name));
...
//        let mut generate_final_close_paren = true;
    ...
    fn visit_dispatch_node(&mut self, dispatch_node: &DispatchNode) -> AstVisitorReturnType {
        self.newline();
        self.add_code(&format!("self._s{}_(e)", dispatch_node.target_state_ref.name));
...
//        self.add_code(&format!("{}",identifier_node.name.lexeme));
    ...
    fn visit_action_decl_node(&mut self, action_decl_node: &ActionDeclNode) -> AstVisitorReturnType {
        ...
        self.indent();
        self.newline();
        self.add_code(&format!("raise NotImplementedError"));
        self.newline_to_string(&mut subclass_code);
        subclass_code.push_str(&format!("#pass"));
        self.outdent();
        self.newline();
