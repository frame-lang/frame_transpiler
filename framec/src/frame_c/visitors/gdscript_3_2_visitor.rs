...
impl GdScript32Visitor {
    ...
    pub fn new(   arcanium:Arcanum
                  ...
                  , comments:Vec<Token>) -> GdScript32Visitor {
            ...
            if self.generate_transition_state {
                ...
                if self.generate_state_context {
                    if self.generate_exit_args {
                        self.add_code(&format!("func _transition_(newState, exitArgs, stateContext):"));
                    } else {
                ...
                if self.generate_state_context {
                    ...
                    self.newline();
                    self.add_code(&format!("var enterEvent = FrameEvent.new(\">\",self._stateContext_.getEnterArgs())"));
                    self.newline();
                } else {
...
//                self.add_code(&format!("}}"));
    ...
    fn generate_state_ref_change_state(&mut self, change_state_stmt_node: &ChangeStateStatementNode) {
        ...
        self.newline();
        self.add_code(&format!("self._changeState_(funcref(self,\"{}\"))",self.format_target_state_name(target_state_name)));
    ...
    fn generate_state_ref_transition(&mut self, transition_statement: &TransitionStatementNode) {
        ...
        if self.generate_state_context {
            self.add_code(&format!("var stateContext = StateContext.new(funcref(self,\"{}\"))", self.generate_state_ref_code(&target_state_name)));
            self.newline();
        }
...
//                code = target_state_vars.clone();
        ...
        if self.generate_state_context {
            if self.generate_exit_args {
                self.add_code(&format!("self._transition_(funcref(self,\"{}\"),{},stateContext)",self.format_target_state_name(target_state_name),exit_args));
            } else {
                self.add_code(&format!("self._transition_(funcref(self,\"{}\"),stateContext)",self.format_target_state_name(target_state_name)));
            }
        } else {
            if self.generate_exit_args {
                self.add_code(&format!("self._transition_(funcref(self,\"{}\"),{})",self.format_target_state_name(target_state_name),exit_args));
            } else {
                self.add_code(&format!("self._transition_(funcref(self,\"{}\"))",self.format_target_state_name(target_state_name)));
            }
        }
    }
...
impl AstVisitor for GdScript32Visitor {
    ...
    fn visit_system_node(&mut self, system_node: &SystemNode) -> AstVisitorReturnType {
        ...
        if self.has_states {
            ...
            self.indent();
            self.newline();
            self.add_code(&format!("self._state_ = funcref(self,\"_s{}_\")", self.first_state_name));

            if self.generate_state_context {
                self.newline();
                self.add_code(&format!("self._stateContext_ = StateContext.new(self._state_)"));
    ...
    fn visit_interface_method_node(&mut self, interface_method_node: &InterfaceMethodNode) -> AstVisitorReturnType {
        ...
        self.newline();
        self.add_code(&format!("var e = FrameEvent.new(\"{}\",{})", method_name_or_alias,params_param_code));
        self.newline();
...
//        self.add_code(&format!("{}",identifier_node.name.lexeme));
    ...
    fn visit_frame_event_part(&mut self, frame_event_part:&FrameEventPart) -> AstVisitorReturnType {
        ...
        match frame_event_part {
            ...
            FrameEventPart::Param {param_tok, is_reference: _is_reference} => self.add_code(&format!("e._parameters[\"{}\"]",param_tok.lexeme)),
    ...
    fn visit_frame_event_part_to_string(&mut self, frame_event_part:&FrameEventPart, output:&mut String) -> AstVisitorReturnType {
        ...
        match frame_event_part {
            ...
            FrameEventPart::Param {param_tok, is_reference: _is_reference} => output.push_str(&format!("e._parameters[\"{}\"]",param_tok.lexeme)),
