...
impl CsVisitorForBob {
    ...
    fn generate_default_actions_block(&mut self) {
        self.newline();
        self.newline();
        self.add_code("//===================== Actions Block ===================//");
        self.newline();
        self.generate_default_actions();
    }

    //* --------------------------------------------------------------------- *//

    fn generate_default_actions(&mut self) {
        self.newline();
        self.add_code("protected virtual void Print_do(object o1) { FrameConsolePrint(o1); }");
        self.newline();
        self.add_code("protected virtual void Print_do(object o1, object o2) { FrameConsolePrint(o1, o2); }");
        self.newline();
        self.add_code("protected virtual void Print_do(object o1, object o2, object o3) { FrameConsolePrint(o1, o2, o3); }");
    }
...
//            self.code.push_str(&*format!("\n{}",(0..self.dent).map(|_| "\t").collect::<String>()));
    ...
    fn generate_state_ref_transition(&mut self, transition_statement: &TransitionStatementNode) {
        ...
        if self.generate_state_context {
            self.add_code(&format!("stateContext = new StateContext({});", state_ref_code));
            self.newline();
        }
...
impl AstVisitor for CsVisitorForBob {
    ...
    fn visit_system_node(&mut self, system_node: &SystemNode) -> AstVisitorReturnType {
        ...
        self.newline();
        self.newline();
        self.add_code(&format!("public partial class {}Base : FrameController {{", system_node.name));
        self.indent();
        self.newline();
        ...
        self.subclass_code.push("/********************".to_string());
        self.subclass_code.push(format!("public partial class {} : {}Base {{",system_node.name,system_node.name));
//        self.subclass_code.push(format!("\tlet that = {}Base.call(this);", system_node.name));
        ...
        if let Some(actions_block_node) = &system_node.actions_block_node_opt {
            ...
            self.generate_default_actions_block();
        }
...
//        self.add_code(&format!("{}",identifier_node.name.lexeme));
    ...
    fn visit_action_decl_node(&mut self, action_decl_node: &ActionNode) -> AstVisitorReturnType {
        ...
        action_code.push_str(&format!("\tprotected virtual {} {}(",action_ret_type, action_name));
        ...
        action_code.push_str(&format!(") {{ throw new NotImplementedException(); }}"));
        self.subclass_code.push(action_code);
