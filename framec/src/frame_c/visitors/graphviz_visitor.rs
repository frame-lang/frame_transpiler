...
impl GraphVizVisitor {
    ...
    fn generate_states(
        ...
        if !is_system_node {
            ...
            if node.children.len() > 0 {
                output.push_str(&self.format_parent_state(&node_name, actual_indent));
            } else {
    ...
    fn format_state(&self, state_name: &str, indent: usize) -> String {
        format!(
            indoc! {r#"
                {}{} [label = <
                {}    <table
                ...
                {}]
            "#},
            self.indent_by_given_input(indent),
            state_name,
            self.indent_by_given_input(indent),
            self.indent_by_given_input(indent),
            self.indent_by_given_input(indent),
    ...
    fn format_parent_state(&self, state_name: &str, indent: usize) -> String {
        format!(
            indoc! {r#"
            {}subgraph cluster_{} {{
            {}    label = <
    ...
    fn indent_by_given_input(&self, indent: usize) -> String {
    ...
    fn generate_state_ref_transition(&mut self, transition_statement: &TransitionStatementNode) {
        ...
        let mut transition_type: TransitionLabelType = TransitionLabelType::Solid;
    ...
    fn format_transition_label(
        &self,
        label: &str,
        transition_type: TransitionLabelType,
    ...
    fn generate_state_stack_pop_transition(
        ...
        self.add_code(&format!(
            "{}Stack[shape={} label={} width={} margin={}]\n",
            self.dent(),
            "\"circle\"",
            "\"H*\"",
            "\"0\"",
        ...
        let label = match &transition_statement.transition_expr_node.label_opt {
            Some(label) => {
                let cleaned = str::replace(label, "|", "&#124;");
                self.format_transition_label(&cleaned, TransitionLabelType::Solid, None)
            }
            None => self.format_transition_label(
                &self.event_handler_msg,
                TransitionLabelType::Solid,
...
impl AstVisitor for GraphVizVisitor {
    ...
    fn visit_system_node(&mut self, system_node: &SystemNode) {
        ...
        let _ = self.compiler_version.clone(); // hack to shut the compiler up
                                               ...
                                               // self.newline();
        self.add_code(&format!("digraph {} {{", self.system_name));
        self.indent();
        self.newline();
        self.add_code(&format!("compound=true"));
        self.newline();
        self.add_code(&format!(
            "node [color={} style={} fillcolor={}]",
            "\"deepskyblue4\"", "\"rounded, filled\"", "\"azure\""
        ));
        self.newline();
        self.add_code(&format!("edge[color={}]\n", "\"red\""));
        ...
        match system_node.get_first_state() {
            Some(x) => {
                self.add_code(&format!(
                    "{}Entry[width={} shape={} style={} fillcolor={} label={}]\n",
                    self.dent(),
                    0.2,
                    "\"circle\"",
                    "\"filled\"",
