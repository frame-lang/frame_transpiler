...
impl CVisitor {
    ...
    fn generate_forward_declarations(&mut self) {
        ...
        self.builder.writeln(&format!("typedef struct {} {}_t;", 
            self.c_config.code.module_state_name,
            self.c_config.code.module_state_name));
        
        // System struct forward declaration
        self.builder.writeln(&format!("typedef struct {} {}_t;", self.system_name, self.system_name));
    ...
    fn generate_module_state_struct(&mut self) {
        ...
        if self.domain_variables.is_empty() {
            self.builder.writeln("int _placeholder; // No domain variables");
        } else {
        ...
        self.builder.writeln(&format!("static {}_t g_module = {{0}};", state_name));
        self.builder.writeln("");
    }
    ...
    fn generate_system_struct(&mut self) {
        ...
        self.builder.writeln(&format!("struct {} {{", self.system_name));
        self.builder.indent();
        
        // Current state enum
        if self.states.is_empty() {
            self.builder.writeln("int current_state; // No states defined");
        } else {
            self.builder.writeln("enum {");
            self.builder.indent();
            for (i, state) in self.states.iter().enumerate() {
                if i == self.states.len() - 1 {
                    self.builder.writeln(&format!("STATE_{}", state.to_uppercase()));
                } else {
                    self.builder.writeln(&format!("STATE_{},", state.to_uppercase()));
                }
            }
        ...
        self.builder.writeln(&format!("{}_t* module;", self.c_config.code.module_state_name));
    ...
    fn generate_module_init(&mut self) {
        ...
        self.builder.writeln(&format!("static void {}(void) {{", init_func));
        self.builder.indent();
        ...
        if self.c_config.features.thread_safe {
            self.builder.writeln("if (pthread_mutex_init(&g_module.mutex, NULL) != 0) {");
            self.builder.indent();
            self.builder.writeln("fprintf(stderr, \"Failed to initialize mutex\\n\");");
        ...
        if !self.domain_variables.is_empty() {
            ...
            for (var_name, var_type) in &self.domain_variables {
                ...
                self.builder.writeln(&format!("g_module.{} = {};", var_name, default_value));
            }
            self.builder.writeln("");
        }
        ...
        self.builder.writeln(&format!("void {}(void) {{", ensure_func));
        self.builder.indent();
        
        if self.c_config.features.thread_safe {
            self.builder.writeln("static pthread_once_t once = PTHREAD_ONCE_INIT;");
            self.builder.writeln(&format!("pthread_once(&once, {});", init_func));
        } else {
        ...
        if self.c_config.features.use_constructor {
            self.builder.writeln(&format!("__attribute__((constructor))"));
            self.builder.writeln(&format!("static void __frame_auto_init(void) {{"));
            self.builder.indent();
            self.builder.writeln(&format!("{}();", init_func));
            self.builder.dedent();
            self.builder.writeln("}");
    ...
    fn generate_system_constructor(&mut self) {
        ...
        self.builder.writeln(&format!("{}_t* {}_new(void) {{", self.system_name, self.system_name.to_lowercase()));
        self.builder.indent();
        
        self.builder.writeln(&format!("{}();", self.c_config.code.ensure_init_name));
        self.builder.writeln("");
        
        self.builder.writeln(&format!("{}_t* self = ({}_t*)malloc(sizeof({}_t));", 
            self.system_name, self.system_name, self.system_name));
        self.builder.writeln("if (!self) {");
        self.builder.indent();
        self.builder.writeln("fprintf(stderr, \"Memory allocation failed\\n\");");
        ...
        if !self.states.is_empty() {
            self.builder.writeln(&format!("self->current_state = STATE_{};", 
    ...
    fn generate_system_destructor(&mut self) {
        ...
        self.builder.writeln(&format!("void {}_free({}_t* self) {{", 
            self.system_name.to_lowercase(), self.system_name));
        self.builder.indent();
    ...
    fn collect_domain_variables(&mut self, domain_block: &DomainBlockNode) {
        for var_decl_rcref in &domain_block.member_variables {
            ...
            if let Some(type_node) = &var_decl.type_opt {
                ...
                self.domain_variables.insert(var_decl.name.clone(), "int".to_string());
            }
        }
    }
    ...
    fn generate_interface_method_c(&mut self, method: &InterfaceMethodNode) {
        ...
        if return_type == "void" {
            ...
            self.builder.writeln(&format!("if (!self) return {};", default_value));
        }
        self.builder.writeln(&format!("{}();", self.c_config.code.ensure_init_name));
        self.builder.writeln("");
    ...
    fn generate_action_method_c(&mut self, action: &ActionNode) {
        ...
        self.builder.writeln(&format!("static {} {}_{} ({}) {{", 
            return_type,
            self.system_name.to_lowercase(),
            action_name,
            params_str
