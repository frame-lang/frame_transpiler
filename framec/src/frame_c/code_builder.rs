...
impl CodeBuilder {
    ...
    pub fn write(&mut self, s: &str) -> &mut Self {
        ...
        for ch in s.chars() {
            ...
            if ch == '\n' {
                ...
                if self.debug {
                    eprintln!("CodeBuilder: Advanced to line {}", self.current_position.line);
                }
            } else {
                self.current_position.column += 1;
                self.at_line_start = false;
    ...
    pub fn merge(&mut self, child: CodeBuilder, frame_line: Option<usize>) -> &mut Self {
        ...
        let line_offset = self.current_position.line - child.mappings.first()
            .map(|m| m.python_line)
            .unwrap_or(1);
    ...
    pub fn write_class(&mut self, name: &str, base: Option<&str>, frame_line: Option<usize>) -> &mut Self {
        if let Some(line) = frame_line {
            self.map_next(line);
        }
...
mod tests {
    ...
    fn test_function_helper() {
        ...
        builder.write_function("test_func", "x, y", false, 5)
