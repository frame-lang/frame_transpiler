    ...
    fn validate(&self, context: &ValidationContext) -> Vec<ValidationIssue> {
        match context.target_language {
            Some(TargetLanguage::Python) => {
                // For Python target we rely on the parser/MixedBody for structural integrity and
                // skip textual unmatched-brace checks to avoid false positives from native code.
                Vec::new()
            }
