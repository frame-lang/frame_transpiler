...
impl MalformedHandlerRule {
    ...
    fn has_malformed_structure(
        ...
        // If this handler was parsed as MixedBody or native-only blocks were captured,
        // rely on the parser's boundary detection instead of textual brace scans.
        if handler.mixed_body.is_some() || !handler.parsed_target_blocks.is_empty() {
            return None;
        }
    ...
    fn extract_handler_text(
        ...
        for (_i, line) in lines.iter().enumerate().skip(start_line.saturating_sub(1)) {
            ...
            if !in_handler
                && (trimmed.starts_with(&format!("{}(", handler_name))
    ...
    fn check_interface_method_coverage(&self, context: &ValidationContext) -> Vec<ValidationIssue> {
        ...
        let interface_methods: Vec<String> =
            if let Some(interface) = &context.ast.interface_block_node_opt {
                interface
                    .interface_methods
                    .iter()
        ...
        if let Some(machine) = &context.ast.machine_block_node_opt {
            for state_ref in &machine.states {
                ...
                let handled_events: std::collections::HashSet<String> = state
                    .evt_handlers_rcref
                    .iter()
                    .map(|handler_ref| {
...
mod tests {
    ...
    fn test_missing_braces_detection() {
        ...
        assert!(
            ...
            "Test case should have missing braces"
        );
    }
