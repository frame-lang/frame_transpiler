    ...
    fn to_serializable(&self) -> Self::Output {
        ...
        for node in &self.call_chain {
            let serializable_node = match node {
                CallChainNodeType::SelfT { .. } => SerializableCallChainNode::Self_,
                ...
                CallChainNodeType::ActionCallT {
                    ...
                    parameters: action_call_expr_node
                        .call_expr_list
                        .exprs_t
                        .iter()
                ...
                CallChainNodeType::OperationCallT {
                    ...
                    parameters: operation_call_expr_node
                        .call_expr_list
                        .exprs_t
                        .iter()
                        .map(|e| expr_to_serializable(e))
                ...
                CallChainNodeType::InterfaceMethodCallT {
                    ...
                    name: interface_method_call_expr_node
                        .identifier
                        .name
                        .lexeme
                        .clone(),
                    parameters: interface_method_call_expr_node
                        .call_expr_list
                        .exprs_t
                        .iter()
                        .map(|e| expr_to_serializable(e))
...
pub fn expr_to_serializable(expr: &ExprType) -> SerializableExpression {
    match expr {
        ...
        ExprType::VariableExprT { var_node } => SerializableExpression::Variable {
            name: var_node.id_node.name.lexeme.clone(),
        },
        ExprType::LiteralExprT { literal_expr_node } => SerializableExpression::Literal {
        ...
        ExprType::ActionCallExprT {
            ...
            arguments: action_call_expr_node
                .call_expr_list
                .exprs_t
                .iter()
                .map(|e| expr_to_serializable(e))
        ...
        ExprType::CallExprT { call_expr_node } => {
            SerializableExpression::Call {
                ...
                resolved_type: call_expr_node
                    .resolved_type
                    .as_ref()
                    .map(|rt| format!("{:?}", rt)),
            }
        }
        ExprType::BinaryExprT { binary_expr_node } => SerializableExpression::Binary {
            ...
            right: Box::new(expr_to_serializable(
                &*binary_expr_node.right_rcref.borrow(),
            )),
        },
        ExprType::UnaryExprT { unary_expr_node } => SerializableExpression::Unary {
        ...
        ExprType::NilExprT => SerializableExpression::Literal {
            value: "None".to_string(),
        },
        ExprType::SystemInstanceExprT { .. } => SerializableExpression::Literal {
            value: "system_instance".to_string(),
        },
        ExprType::SystemTypeExprT { .. } => SerializableExpression::Literal {
            value: "system_type".to_string(),
        },
        ExprType::FrameEventExprT { .. } => SerializableExpression::Literal {
    ...
    SerializableSystem {
        ...
        interface: system
            ...
            .map(|i| {
                i.interface_methods
                    .iter()
                    .map(|m| serialize_interface_method(m))
                    .collect()
            })
            .unwrap_or_default(),
        operations: system
        ...
        actions: system
            ...
            .map(|a| {
                a.actions
                    .iter()
                    .map(|action| serialize_action(action))
                    .collect()
            })
            .unwrap_or_default(),
        domain: system
            ...
            .map(|d| {
                d.member_variables
                    .iter()
                    .map(|var| serialize_domain_var(var))
                    .collect()
            })
            .unwrap_or_default(),
    }
...
fn serialize_state(state: &Rc<RefCell<StateNode>>) -> SerializableState {
    ...
    SerializableState {
        ...
        event_handlers: state_borrowed
            .evt_handlers_rcref
            .iter()
            .map(|h| serialize_event_handler(h))
            .collect(),
...
fn serialize_interface_method(
    method: &Rc<RefCell<InterfaceMethodNode>>,
...
pub fn ast_summary(module: &FrameModule) -> String {
    let systems_summary: Vec<String> = module
        .systems
        .iter()
        .map(|s| {
            let states_count = s
    ...
    let functions_summary: Vec<String> = module
        .functions
        .iter()
        .map(|f| {
    ...
    format!(
        "AST Summary:\n  Systems ({}): {}\n  Functions ({}): {}\n  Enums ({}): {}",
        module.systems.len(),
        if systems_summary.is_empty() {
            "None".to_string()
        } else {
            systems_summary.join(", ")
        },
        module.functions.len(),
        if functions_summary.is_empty() {
            "None".to_string()
        } else {
...
pub fn generate_line_map(module: &FrameModule) -> String {
    ...
    for system in &module.systems {
        ...
        if let Some(machine) = &system.machine_block_node_opt {
            for state in &machine.states {
                ...
                lines.push(format!(
                    "  State '{}' at line {}",
                    state_borrowed.name, state_borrowed.line
                ));
        ...
        if let Some(operations) = &system.operations_block_node_opt {
            for op in &operations.operations {
                ...
                lines.push(format!(
                    "  Operation '{}' at line {}",
                    op_borrowed.name, op_borrowed.terminator_expr.line
                ));
            }
        }
        ...
        if let Some(actions) = &system.actions_block_node_opt {
            for action in &actions.actions {
                ...
                lines.push(format!(
                    "  Action '{}' at line {}",
                    action_borrowed.name, action_borrowed.terminator_expr.line
                ));
            }
        }
    }
    ...
    for function in &module.functions {
        ...
        lines.push(format!(
            "Function '{}' at line {}{}",
            f_borrowed.name, f_borrowed.line, async_marker
        ));
    }
