...
impl<'a> SemanticCallAnalyzer<'a> {
    ...
    pub fn resolve_call(&self, call_expr: &CallExprNode) -> ResolvedCallType {
        ...
        if std::env::var("FRAME_TRANSPILER_DEBUG").is_ok()
            && (identifier.starts_with('_') || identifier == "test_action")
        {
        ...
        if identifier.starts_with('_') {
            if let Some(system) = self.current_system {
                ...
                if std::env::var("FRAME_TRANSPILER_DEBUG").is_ok() {
                    eprintln!(
                        "DEBUG v0.66: Resolving {} as Action in system {}",
                        identifier, system
                    );
                }
                ...
                if std::env::var("FRAME_TRANSPILER_DEBUG").is_ok() {
                    eprintln!(
                        "DEBUG v0.66: Cannot resolve {} as Action - no current_system",
                        identifier
                    );
                }
            }
        ...
        if let Some(system) = self.current_system {
            if std::env::var("FRAME_TRANSPILER_DEBUG").is_ok() && identifier == "test_action" {
                eprintln!(
                    "DEBUG v0.66: In system context '{}', checking identifier '{}'",
                    system, identifier
                );
            }
            ...
            if std::env::var("FRAME_TRANSPILER_DEBUG").is_ok() && identifier == "test_action" {
                eprintln!(
                    "DEBUG v0.66: Is '{}' an action in system '{}': {}",
            ...
            if std::env::var("FRAME_TRANSPILER_DEBUG").is_ok() && identifier == "test_action" {
                eprintln!(
                    "DEBUG v0.66: No current system context for identifier '{}'",
                    identifier
                );
            }
        }
            ...
            if let Some(ref operations) = sys.operations_block_symbol_opt {
                ...
                for (_name, symbol_rcref) in &ops_symtab.borrow().symbols {
                    if let SymbolType::OperationScope {
                        operation_scope_symbol_rcref: _,
                    } = &*symbol_rcref.borrow()
                    {
            ...
            if let Some(ref actions) = sys.actions_block_symbol_opt {
                ...
                for (_name, symbol_rcref) in &actions_symtab.borrow().symbols {
                    ...
                    } = &*symbol_rcref.borrow()
                    {
                        ...
                        if std::env::var("FRAME_TRANSPILER_DEBUG").is_ok()
                            && action_name.contains("process")
                        {
                            eprintln!(
                                "DEBUG v0.66: Comparing action '{}' with '{}'",
                                action_scope_name, action_name
                            );
                        }
                        // v0.66: Check both with and without underscore prefix
                        if action_scope_name == action_name
                            || (action_scope_name.starts_with('_')
                                && &action_scope_name[1..] == action_name)
                            || (action_name.starts_with('_')
                                && action_scope_name == &action_name[1..])
                        {
                            return true;
                        }
                    }
    ...
    pub fn enter_system(&mut self, system_name: &'a str) {
        ...
        if std::env::var("FRAME_TRANSPILER_DEBUG").is_ok() {
            eprintln!(
                "DEBUG v0.66: SemanticCallAnalyzer::enter_system({})",
                system_name
            );
        }
