        ...
        while i<n {
            if at_sol {
                ...
                if j<n && (starts_kw(bytes,j,b"package") || starts_kw(bytes,j,b"import")) {
                    let stmt_start = line_start; let mut k=j; let mut in_s=false; let mut esc=false; let mut block=false;
                    while k<n {
                        ...
                        match b { b'"' => { in_s=true; k+=1; }, b';' => { spans.push(RegionSpan{ start: stmt_start, end: k }); k+=1; i=k; break; }, b'\n' => { k+=1; }, _ => { k+=1; } }
                    }
                    if k>=n { spans.push(RegionSpan{ start: stmt_start, end: n }); i=n; }
                    continue;
                }
                break;
