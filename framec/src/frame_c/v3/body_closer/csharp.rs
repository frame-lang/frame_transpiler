...
pub struct BodyCloserCsV3;

impl BodyCloserV3 for BodyCloserCsV3 {
            ...
            match bytes[i] {
                ...
                b'@' => {
                    ...
                    if i+1<n && bytes[i+1]==b'"' { i+=2; // verbatim
                        loop { if i>=n { return Err(CloseErrorV3{ kind: CloseErrorV3Kind::UnterminatedString, message: "unterminated verbatim string".into() }); }
                        ...
                        loop { if i>=n { return Err(CloseErrorV3{ kind: CloseErrorV3Kind::UnterminatedString, message: "unterminated interp verbatim".into() }); }
                            if i+1<n && bytes[i]==b'"' && bytes[i+1]==b'"' { i+=2; continue; }
                            if bytes[i]==b'"' { i+=1; break; }
                            i+=1;
                        }
                ...
                b'$' => {
                    ...
                    if quotes>=3 { // raw string
                        i = k; // inside raw
                        loop {
                            if i>=n { return Err(CloseErrorV3{ kind: CloseErrorV3Kind::UnterminatedRawString, message: "unterminated raw".into() }); }
                        ...
                        loop { if i>=n { return Err(CloseErrorV3{ kind: CloseErrorV3Kind::UnterminatedString, message: "unterminated interp string".into() }); }
