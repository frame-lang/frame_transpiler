...
impl MultiFileCompiler {
    ...
    pub fn compile(&mut self, entry_file: &Path) -> ModuleResult<String> {
        if std::env::var("FRAME_TRANSPILER_DEBUG").is_ok() {
            eprintln!(
                "DEBUG: Starting multi-file compilation from {:?}",
                entry_file
            );
        }
        ...
        if std::env::var("FRAME_TRANSPILER_DEBUG").is_ok() {
            eprintln!(
                "DEBUG: Compilation order has {} modules",
    ...
    fn parse_module(&mut self, file_path: &Path) -> ModuleResult<CompiledModule> {
        ...
        if has_errors {
            return Err(ModuleError::new(
                ModuleErrorKind::ParseError { error: errors },
                file_path.display().to_string(),
            ));
        }
        ...
        for (path, module) in &self.parsed_modules {
            for dep_path in &module.dependencies {
                self.dependency_graph
                    .add_dependency(path.clone(), dep_path.clone());
            }
        }
            ...
            for import in &module.ast.imports {
                match &import.import_type {
                    ImportType::FrameModule {
                        ...
                        let target_module =
                            self.parsed_modules.get(&resolved.fs_path).ok_or_else(|| {
                                ModuleError::new(
