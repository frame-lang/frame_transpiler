...
impl ModuleLinker {
    ...
    fn link_concatenation(&mut self) -> ModuleResult<String> {
        ...
        // Add header comments
        output.push_str("# Generated by Frame Transpiler v0.66\n");
        ...
        let framec_version = if let Some((_, first_code)) = module_codes.first() {
            if let Some(line) = first_code.lines().find(|l| l.starts_with("#Emitted from framec")) {
                ...
                "#Emitted from framec_v0.76.1".to_string()
            }
        } else {
            "#Emitted from framec_v0.76.1".to_string()
        };
        ...
        // Add main execution guard if there's a main function
        if self.has_main_function(&module_codes) {
    ...
    /// Generate consolidated import section
    ...
    /// Filter module code to remove imports, runtime classes, and version comments
    fn filter_module_code(&self, code: &str) -> String {
        ...
        for line in code.lines() {
            ...
            if trimmed.starts_with("#Emitted from framec") {
                skip_next_blank = true;
                continue;
            }
            
            // Skip Frame import comments (must check before regular imports)
            if line.contains("# Frame import") {
                skip_next_blank = true;
                continue;
            }
            ...
            // Check if we're entering a FrameEvent class definition
            if line.starts_with("class FrameEvent:") {
                in_frame_event = true;
                skip_next_blank = true;
                continue;
            }
            
            // Check if we're entering a FrameCompartment class definition
            if line.starts_with("class FrameCompartment:") {
                in_frame_compartment = true;
                skip_next_blank = true;
                continue;
            }
            
            // If we're in a runtime class, check for end
