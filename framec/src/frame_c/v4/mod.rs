...
pub fn compile_module(content_str: &str, lang: TargetLanguage) -> Result<String, RunError> {
            ...
            let exps: Vec<String> = {
                ...
                for r in &scan.regions {
                    if let crate::frame_c::v4::native_region_scanner::RegionV3::FrameSegment{ indent, .. } = r {
                        ...
                        if std::env::var("FRAME_DEBUG_FACADE").ok().as_deref() == Some("1") {
                            let kind = match m { crate::frame_c::v4::mir::MirItemV3::Transition{..} => "Transition", crate::frame_c::v4::mir::MirItemV3::Forward{..} => "Forward", crate::frame_c::v4::mir::MirItemV3::StackPush{..} => "StackPush", crate::frame_c::v4::mir::MirItemV3::StackPop{..} => "StackPop" };
                ...
                if std::env::var("FRAME_DEBUG_FACADE").ok().as_deref() == Some("1") {
                    ...
                    for (idx, r) in scan.regions.iter().enumerate() {
                        match r { crate::frame_c::v4::native_region_scanner::RegionV3::FrameSegment{ kind, .. } => eprintln!("[facade-compile] region[{idx}] = Frame({:?})", kind), _ => {} }
                    }
        ...
        if exec_mir.is_none() {
            for b in &parts.bodies {
                ...
                let scan = match lang {
                    ...
                    TargetLanguage::Java => nscan::java::NativeRegionScannerJavaV3.scan(body_src.as_bytes(), 0),
                    TargetLanguage::Rust => nscan::rust::NativeRegionScannerRustV3.scan(body_src.as_bytes(), 0),
                    _ => Ok(crate::frame_c::v4::native_region_scanner::ScanResultV3 { close_byte: body_src.len().saturating_sub(1), regions: Vec::new() })
                }.unwrap_or(crate::frame_c::v4::native_region_scanner::ScanResultV3 { close_byte: body_src.len().saturating_sub(1), regions: Vec::new() });
        ...
        let program = match lang {
            TargetLanguage::Python3 => {
                ...
                if let Some(ref mirv) = exec_mir {
                    use crate::frame_c::v4::expander::PyExpanderV3;
                    for m in mirv { let s = PyExpanderV3.expand(m, 4, system_name.as_deref()); p.push_str(&s); }
                } else if let Some(src) = exec_body_src.as_ref() {
                    let scan = match nscan::python::NativeRegionScannerPyV3.scan(src.as_bytes(), 0) { Ok(s) => s, Err(_) => crate::frame_c::v4::native_region_scanner::ScanResultV3{ close_byte: src.len().saturating_sub(1), regions: Vec::new() } };
            ...
            TargetLanguage::C => {
                ...
                if let Some(ref mirv) = exec_mir {
                    use crate::frame_c::v4::expander::CFacadeExpanderV3;
                    for m in mirv { let s = CFacadeExpanderV3.expand(m, 4, None); p.push_str(&s); }
                } else if let Some(src) = exec_body_src.as_ref() {
                    let scan = match nscan::c::NativeRegionScannerCV3.scan(src.as_bytes(), 0) { Ok(s) => s, Err(_) => crate::frame_c::v4::native_region_scanner::ScanResultV3{ close_byte: src.len().saturating_sub(1), regions: Vec::new() } };
                    let mir = MirAssemblerV3.assemble(src.as_bytes(), &scan.regions).unwrap_or_else(|_| Vec::new());
                    use crate::frame_c::v4::expander::CFacadeExpanderV3;
            ...
            TargetLanguage::Cpp => {
                ...
                p.push_str("    FrameCompartment compartment = frame_compartment_new(\"__S_state_A\");\n");
                if let Some(ref mirv) = exec_mir {
                    use crate::frame_c::v4::expander::CppFacadeExpanderV3;
                    for m in mirv { let s = CppFacadeExpanderV3.expand(m, 4, None); p.push_str(&s); }
                } else if let Some(src) = exec_body_src.as_ref() {
                    let scan = match nscan::cpp::NativeRegionScannerCppV3.scan(src.as_bytes(), 0) { Ok(s) => s, Err(_) => crate::frame_c::v4::native_region_scanner::ScanResultV3{ close_byte: src.len().saturating_sub(1), regions: Vec::new() } };
                    let mir = MirAssemblerV3.assemble(src.as_bytes(), &scan.regions).unwrap_or_else(|_| Vec::new());
                    use crate::frame_c::v4::expander::CppFacadeExpanderV3;
            ...
            TargetLanguage::Java => {
                ...
                if let Some(ref mirv) = exec_mir {
                    use crate::frame_c::v4::expander::JavaFacadeExpanderV3;
                    for m in mirv { let s = JavaFacadeExpanderV3.expand(m, 4, None); p.push_str(&s); }
                } else if let Some(src) = exec_body_src.as_ref() {
                    let scan = match nscan::java::NativeRegionScannerJavaV3.scan(src.as_bytes(), 0) { Ok(s) => s, Err(_) => crate::frame_c::v4::native_region_scanner::ScanResultV3{ close_byte: src.len().saturating_sub(1), regions: Vec::new() } };
                    let mir = MirAssemblerV3.assemble(src.as_bytes(), &scan.regions).unwrap_or_else(|_| Vec::new());
                    use crate::frame_c::v4::expander::JavaFacadeExpanderV3;
            ...
            TargetLanguage::CSharp => {
                ...
                if let Some(ref mirv) = exec_mir {
                    ...
                    let scan = match nscan::csharp::NativeRegionScannerCsV3.scan(src.as_bytes(), 0) { Ok(s) => s, Err(_) => crate::frame_c::v4::native_region_scanner::ScanResultV3{ close_byte: src.len().saturating_sub(1), regions: Vec::new() } };
                    let mir = MirAssemblerV3.assemble(src.as_bytes(), &scan.regions).unwrap_or_else(|_| Vec::new());
                    use crate::frame_c::v4::expander::CFacadeExpanderV3;
                    for m in &mir { let s = CFacadeExpanderV3.expand(m, 4, None); p.push_str(&s); }
                }
    ...
    if emit_body_only {
        ...
        if compile_runtimable && !is_module {
            ...
            match lang {
                TargetLanguage::Python3 => {
                    ...
                    let module_ast = crate::frame_c::v4::system_parser::SystemParserV3::parse_module(bytes, TargetLanguage::Python3);
                    let iface_parser = crate::frame_c::v4::interface_parser::InterfaceParserV3;
                    ...
                    for (idx, b) in parts.bodies.iter().enumerate() {
                        ...
                        match b.kind {
                            crate::frame_c::v4::validator::BodyKindV3::Handler => {
                            ...
                            crate::frame_c::v4::validator::BodyKindV3::Action => {
                            ...
                            crate::frame_c::v4::validator::BodyKindV3::Operation => {
                            ...
                            crate::frame_c::v4::validator::BodyKindV3::Function => {
                                ...
                                use crate::frame_c::v4::python_transpiler::PythonTranspilerV3;
                                let transpiler = PythonTranspilerV3;
                                // Re-enable indentation validation now that Frame expansion is fixed
                                let transpiled_body = match transpiler.transpile_function_body(spliced) {
                                    Ok(body) => body,
                                    Err(e) => {
                                        eprintln!("Error: Python indentation validation failed in {}: {}", fname, e);
                                        // Fall back to unchecked version if validation fails
                                        transpiler.transpile_function_body_unchecked(spliced)
                                    }
                                };
                                
                                // Normalize indentation like we do for actions
                                let mut min_indent: Option<usize> = None;
                    ...
                    fn normalize_py_handler_lines(
                        ...
                        // compiled into `crate::frame_c::v4::machines`.
                        crate::frame_c::v4::machines::run_indent_normalizer(
                            lines,
                            flags_is_expansion,
                            flags_is_comment,
                            pad,
                ...
                TargetLanguage::TypeScript => {
                    ...
                    let module_ast = crate::frame_c::v4::system_parser::SystemParserV3::parse_module(bytes, TargetLanguage::TypeScript);
                    let iface_parser = crate::frame_c::v4::interface_parser::InterfaceParserV3;
                    ...
                    use crate::frame_c::v4::import_scanner::ImportScannerV3;
                    let import_scanner = crate::frame_c::v4::import_scanner::python::ImportScannerPyV3;
                        ...
                        match b.kind {
                            crate::frame_c::v4::validator::BodyKindV3::Handler => {
                                ...
                                if let Some(hs) = b.header_span {
                                    ...
                                    if let Some((_, meta)) =
                                        crate::frame_c::v4::interface_parser::parse_interface_header_meta(hdr)
                                    {
                            ...
                            crate::frame_c::v4::validator::BodyKindV3::Action => {
                                ...
                                let (is_async, params, ret_type_opt) = if let Some(hs) = b.header_span {
                                    ...
                                    let ret_ty = crate::frame_c::v4::interface_parser::parse_interface_header_meta(core)
                ...
                TargetLanguage::Rust => {
                    ...
                    let domain_fields_rs = crate::frame_c::v4::rust_domain_scanner::scan_rs_domain_fields(bytes);
                    ...
                    for (idx, b) in parts.bodies.iter().enumerate() {
                        if let crate::frame_c::v4::validator::BodyKindV3::Handler = b.kind {
                            let hname = b.owner_id.as_deref().unwrap_or("handler").to_string();
                        ...
                        } else if matches!(b.kind, crate::frame_c::v4::validator::BodyKindV3::Action | crate::frame_c::v4::validator::BodyKindV3::Operation) {
                            let name = b.owner_id.as_deref().unwrap_or_else(|| {
                                if matches!(b.kind, crate::frame_c::v4::validator::BodyKindV3::Action) {
                                    "action"
                                } else {
                                    "operation"
                                }
                    ...
                    let validator = crate::frame_c::v4::validator::ValidatorV3;
        ...
        if std::env::var("FRAME_MAP_TRAILER").ok().as_deref() == Some("1") {
            use crate::frame_c::v4::splice::OriginV3;
            ...
            for b in &parts.bodies {
                ...
                }.unwrap_or(crate::frame_c::v4::native_region_scanner::ScanResultV3 { close_byte: body_bytes.len().saturating_sub(1), regions: Vec::new() });
                let sys_ctx = system_name.as_deref();
                let (mir, _parse_issues) = MirAssemblerV3.assemble_collect(body_bytes, &scan.regions);
                let exps: Vec<String> = {
                    use crate::frame_c::v4::expander::*; let mut v=Vec::new(); let mut mi=0usize; for r in &scan.regions { if let crate::frame_c::v4::native_region_scanner::RegionV3::FrameSegment{ indent, .. } = r { if mi>=mir.len(){break;} let m=&mir[mi]; mi+=1; let s = match lang { TargetLanguage::Python3 => PyExpanderV3.expand(m, *indent, sys_ctx), TargetLanguage::TypeScript => TsExpanderV3.expand(m, *indent, sys_ctx), TargetLanguage::CSharp => CExpanderV3.expand(m, *indent, sys_ctx), TargetLanguage::C => CExpanderV3.expand(m, *indent, sys_ctx), TargetLanguage::Cpp => CppExpanderV3.expand(m, *indent, sys_ctx), TargetLanguage::Java => JavaExpanderV3.expand(m, *indent, sys_ctx), TargetLanguage::Rust => RustExpanderV3.expand(m, *indent, sys_ctx), _ => String::new(), }; v.push(s); } } v };
            ...
            if std::env::var("FRAME_DEBUG_MANIFEST").ok().as_deref() == Some("1") {
                ...
                if let Some((sys_name, _)) = arc_for_ctx.systems.iter().next() {
                    ...
                    let outline_start = parts.imports.last().map(|s| s.end).or(parts.prolog.as_ref().map(|p| p.end)).unwrap_or(0);
                    let (items, _issues) = crate::frame_c::v4::outline_scanner::OutlineScannerV3.scan_collect(bytes, outline_start, lang);
                    ...
                    let mut hf = true;
                    for it in &items {
                        if let crate::frame_c::v4::validator::BodyKindV3::Handler = it.kind {
        ...
        if std::env::var("FRAME_NATIVE_SYMBOL_SNAPSHOT").ok().as_deref() == Some("1") {
            ...
            let mut first = true;
            for it in &items {
                if matches!(it.kind, crate::frame_c::v4::validator::BodyKindV3::Handler) {
        ...
        // Structured errors JSON trailer for the V3 module compile path.
        {
            ...
            let arcanum = crate::frame_c::v4::arcanum::build_arcanum_from_module_ast(bytes, &module_ast);
            ...
            for b in &parts.bodies {
                ...
                let scan = match lang {
                    ...
                    TargetLanguage::Java => nscan::java::NativeRegionScannerJavaV3.scan(body_bytes, 0),
                    TargetLanguage::Rust => nscan::rust::NativeRegionScannerRustV3.scan(body_bytes, 0),
                    _ => Ok(crate::frame_c::v4::native_region_scanner::ScanResultV3 { close_byte: body_bytes.len().saturating_sub(1), regions: Vec::new() })
                }.unwrap_or(crate::frame_c::v4::native_region_scanner::ScanResultV3 { close_byte: body_bytes.len().saturating_sub(1), regions: Vec::new() });
                ...
                if matches!(b.kind, crate::frame_c::v4::validator::BodyKindV3::Action | crate::frame_c::v4::validator::BodyKindV3::Operation) {
                    let pol = crate::frame_c::v4::validator::ValidatorPolicyV3 { body_kind: Some(b.kind) };
                ...
                if validator.has_machine_section(bytes, outline_start) {
                    if matches!(b.kind, BodyKindV3::Handler | BodyKindV3::Function | BodyKindV3::Unknown) {
                        if mir.iter().any(|m| matches!(m, crate::frame_c::v4::mir::MirItemV3::Forward { .. })) {
                            ...
                            if !ok_parent {
                                issues.push(crate::frame_c::v4::validator::ValidationIssueV3{ message: "E403: Cannot forward to parent: no parent available".into() });
                            }
                        }
                    }
                }
                // E406/E407: system.method/system.return usage
                if matches!(b.kind, crate::frame_c::v4::validator::BodyKindV3::Handler | crate::frame_c::v4::validator::BodyKindV3::Action | crate::frame_c::v4::validator::BodyKindV3::Operation) {
                        ...
                        if let Ok(diags) = facade.parse(&spliced.text) {
                            for d in diags {
                                if let Some((_origin, src)) = spliced.map_spliced_range_to_origin(d.start, d.end) {
                                    issues.push(crate::frame_c::v4::validator::ValidationIssueV3{ message: format!("E500: native syntax (frame:{}-{}): {}", src.start, src.end, d.message) });
                                } else {
                                    issues.push(crate::frame_c::v4::validator::ValidationIssueV3{ message: format!("E500: native syntax: {}", d.message) });
                                }
                            }
                        }
                    }
