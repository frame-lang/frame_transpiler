...
- MixedBody/MIR: Ordered sequence of `NativeText` spans and `MirDirective` nodes when native bodies contain Frame directives.
...
1) Frame symbols remain Frame‑owned
   ...
   - Frame directives inside native bodies (MIR) reference Frame symbols only (e.g., `$State`, stack ops). Directive arguments are currently captured as strings; future work may associate parsed target expressions.
...
4) MixedBody drives emission; target AST validates
   - Mixed bodies: visitors emit `NativeText` verbatim and expand `MirDirective` using deterministic glue; we do not need a native code printer for glue.
...
Resolution rules for directive arguments [future]
- If directive args contain native expressions (e.g., `-> $S1(a, this.count)`), we keep them as strings in MIR, and emit them directly in glue. In future we may parse these expressions via the target parser to enable better diagnostics without changing emission.
...
- Mixed TS handler: Segmenter → MixedBody { NativeText + MirDirective } → visitor emits verbatim + glue; Arcanum resolves Frame symbols used by directives.
