...
- Delegating each detected Frame statement to a tiny, statement‑specific mini‑parser to produce MIR (Mixed Intermediate Representation) statements.
...
2) Streaming scan (O(n), byte‑based)
   ...
   - Accumulate native text between SOLs. When a SOL token is in FIRST(FrameStatement), flush the native slice and delegate to a Frame‑statement mini‑parser.

3) Mini‑parsers (per Frame statement)
   ...
   - On parse failure, emit a statement‑specific error (e.g., “Missing `)` in state args”), with `frame_line` and the line’s snippet.

4) MixedBody assembly
   - Append `MixedBodyItem::NativeText` (or `NativeAst` if native parser succeeds) for native runs and `MixedBodyItem::Frame` for each MIR Frame statement.
   - If no Frame statements are found, attach a single native span (prefer native AST when available).
...
- Mini‑parsers produce Frame‑statement‑specific errors with frame lines and helpful suggestions.
...
2) Implement mini‑parsers
   - One per Frame‑statement kind; consume within the line; return MIR and the consumed span; classify failures.

3) Replace segmenters’ inner loops
   - Keep the outer API (returns `Vec<BodySegment>`); inside, use SOLIndex + mini‑parsers instead of per‑char Frame‑statement detection.
...
5) Tests
   - Positive and negative fixtures for each Frame‑statement kind at SOL.
   - Strings/templates/f‑strings containing Frame‑statement‑like tokens must not trigger SOLIndex.
...
- Errors are specific and actionable (unterminated constructs, malformed Frame statements).
- Scanning is linear and free of pathological blow‑ups on large bodies.

## Notes

- This design formalizes the current approach (DPDA for closers + SOL detection) and simplifies Frame‑statement handling via FIRST + mini‑parsers. It is correctness‑preserving and generally faster, while keeping behavior deterministic and well‑tested.
