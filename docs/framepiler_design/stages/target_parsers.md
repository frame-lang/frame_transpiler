...
Purpose
- Parse native target language bodies when they contain only native statements (no Frame statements), producing structured AST blocks for validation and emission. Target parsers are validators and structure providers; they do not own Frame semantics.
...
- TypeScript (SWC)
  ...
  - Emission: In the current B2 strategy, we keep custom visitors for Frame‑statement glue and emit native spans verbatim. The SWC AST remains available for optional validation and future codegen improvements.
...
- The scanner/segmenter ensures Frame statements (e.g., `->`, `=> $^`, `$$[+/-]`) are never passed to target parsers; native bodies that contain Frame statements are handled by the NativeRegionSegmenter + MixedBody/MIR path.
...
- Target parsers do not interleave Frame semantics; any Frame‑statement presence defers to segmenter/MIR.

Validation
- Compile‑only fixtures exercising native constructs (arrows, callbacks, imports, comprehensions) without Frame statements validate parser acceptability.
- Negative fixtures ensure Frame-statement tokens cause segmentation rather than target parsing.
